...
export function createSkidServices(){
 ...
 for(const [j,x] of [-.38,0,.38].entries()){
  ...
  const h=item(`AS-HOSE-${j+1}`,'pipe','氣洗軟管',[0,0,0],{material:'額定氣體軟管待選',spanMm:109.27,cutLengthMm:null,od:10,id:6,note:'109.27 mm 為原圖弧線近似跨度，不含接入與裝配餘量；網頁路由為示意。'});const curve=new THREE.CatmullRomCurve3([v([x,.18,0]),v([x,.16,-.02]),v([x,.155,-.07]),v([x,.155,-.125])]);mesh(h,new THREE.TubeGeometry(curve,24,.005,8,false),[0,0,0],gas);
 }
 // Extensions have their own spans; never included in the original 6.282 m subtotal.
 function extension(tag,service,a,b,material='PVC-U 暫定',r=.055){const o=item(tag,'extension',service+'新增連接段',[0,0,0],{material,spanMm:Math.round(v(a).distanceTo(v(b))*1000),cutLengthMm:null,note:'新增網頁配置跨度，含閥件／儀表占位，非裁管長度；尚未同步回DWG。'});pipe(o,a,b,r,material.startsWith('PVC')?pvc:gas);}
 ...
 const sample=item('SAMPLE-PW-01','extension','產水濁度取樣／排樣管',[0,0,0],{material:'取樣管材待選',cutLengthMm:null,note:'僅旁流量測路由概念，不提供下料長度。排樣處置及調流／隔離配套待確認。'});pipe(sample,[.64,2.14,-.327],[.64,1.3,-.327],.004,cyan);pipe(sample,[.64,1.3,-.327],[.79,1.3,.22],.004,cyan);pipe(sample,[.79,1.10,.22],[.79,.9,.22],.004,cyan);
