...
function selectService(tag){const o=services.items.find(x=>x.tag===tag);if(!o)return;selectedService=o;services.select(tag);$('service-name').textContent=`${tag} · ${o.name}`;$('service-material').textContent=o.details.material;$('service-span').textContent=o.details.spanMm===undefined?'—':`${o.details.spanMm} mm`;$('service-cut').textContent=spanLabel(o);$('service-spec').textContent=o.details.od?`模型 OD ${o.details.od} / ID ${o.details.id??'待選'} mm；DN及管制待定`:o.details.unit?`單位 ${o.details.unit}；量程待確認；讀值 —（未連線）`:'DN、壓力等級及安裝尺寸待確認';$('service-note').textContent=o.details.note;$('service-focus').hidden=false;document.querySelectorAll('[data-service]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.service===tag)));}
...
const lengthGroups=[['PVC-U 水管',o=>o.details.material.startsWith('PVC')],['氣洗總管與連接段',o=>o.tag==='AS-H01'||o.tag==='AS-EXT-01'],['氣洗軟管',o=>o.tag.startsWith('AS-HOSE')],['濁度取樣／排樣管',o=>o.tag==='SAMPLE-PW-01']];
lengthGroups.forEach(([name,predicate])=>{const rows=services.items.filter(o=>Number.isFinite(o.details.estimatedLengthMm)&&predicate(o));const line=document.createElement('div');const label=document.createElement('span');label.textContent=name;const total=document.createElement('strong');total.textContent='約 '+(rows.reduce((a,o)=>a+o.details.estimatedLengthMm,0)/1000).toFixed(2)+' m';line.append(label,total);$('length-summary').appendChild(line);});
...
$('download-materials').onclick=()=>{const rows=[['編號','分類','名稱','數量','材質（暫定）','模型跨度mm','估算長度mm（取整10mm）','模型ODmm','模型IDmm','儀表單位','讀值','規格狀態'],...services.items.map(o=>[o.tag,types[o.type],o.name,1,o.details.material,o.details.spanMm??'',o.details.estimatedLengthMm??'',o.details.od??'待確認',o.details.id??'待確認',o.details.unit??'',o.type==='instrument'?'未連線':'',o.details.note])];const csv='\ufeff'+rows.map(row=>row.map(cell=>'"'+String(cell).replaceAll('"','""')+'"').join(',')).join('\r\n');const url=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download='UF0915E_SKID_materials_PROVISIONAL.csv';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
