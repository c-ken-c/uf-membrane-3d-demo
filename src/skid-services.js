...
export function createSkidServices(){
 ...
 for(const [j,x] of [-.38,0,.38].entries()){
  for(const [code,y,axis] of [['FW',.14,'y'],['PW',2.14,'y'],['CW',2.14,'y']]){const z=code==='CW'?0:-.327;const o=item(`TEE-${code}-${j+1}`,'fitting','總管異徑三通',[x,y,z],{material:'PVC-U 暫定',note:'分支三通外形示意；主管模型OD110／支管OD60，實際DN與承口規格待定。'});pipe(o,[-.072,0,0],[.072,0,0],.061);pipe(o,[0,0,0],[0,code==='FW'?.07:-.07,0],.035);collar(o,[-.068,0,0],.06);collar(o,[.068,0,0],.06);}
  ...
  const h=item(`AS-HOSE-${j+1}`,'pipe','氣洗軟管',[0,0,0],{material:'額定氣體軟管待選',spanMm:109.27,cutLengthMm:null,od:10,id:6,note:'依原圖彎曲路由估計109.27 mm，每條先抓110 mm，不另加裝配餘量。'});const curve=new THREE.CatmullRomCurve3([v([x,.18,0]),v([x,.16,-.02]),v([x,.155,-.07]),v([x,.155,-.125])]);mesh(h,new THREE.TubeGeometry(curve,24,.005,8,false),[0,0,0],gas);
 }
 // Extensions have their own spans; never included in the original 6.282 m subtotal.
 function extension(tag,service,a,b,material='PVC-U 暫定',r=.055){const o=item(tag,'extension',service+'新增連接段',[0,0,0],{material,od:r*2000,spanMm:Math.round(v(a).distanceTo(v(b))*1000),cutLengthMm:null,note:'新增連接段依相同SKID模型比例估長，包含閥件／儀表占位。'});pipe(o,a,b,r,material.startsWith('PVC')?pvc:gas);}
 ...
 const sample=item('SAMPLE-PW-01','extension','產水濁度取樣／排樣管',[0,0,0],{material:'取樣管材待選',od:8,spanMm:Math.round((.84+Math.hypot(.15,.547)+.2)*1000),cutLengthMm:null,note:'取樣與排樣管依目前畫面路由比例估長，含0.84m取樣下降段、斜接段及0.20m排樣段。'});pipe(sample,[.64,2.14,-.327],[.64,1.3,-.327],.004,cyan);pipe(sample,[.64,1.3,-.327],[.79,1.3,.22],.004,cyan);pipe(sample,[.79,1.10,.22],[.79,.9,.22],.004,cyan);
 for(const o of items){if(['pipe','extension'].includes(o.type)&&Number.isFinite(o.details.spanMm)){o.details.estimatedLengthMm=Math.round(o.details.spanMm/10)*10;o.details.estimateBasis='依SKID比例及管段路由，四捨五入至10 mm，含管件占位；未加損耗';}}
